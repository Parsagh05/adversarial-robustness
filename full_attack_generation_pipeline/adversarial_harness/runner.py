        ...
        if scope == "per_image":
            ...
            for group_name, group_fit_samples in group_progress:
                ...
                with tqdm(
                    ...
                    def report(
                        step: int, total: int, metrics: Dict[str, float]
                    ) -> None:
                        optimization_progress.update(step - optimization_progress.n)
                        postfix = {
                            "batch_post": f"{metrics['total_loss']:.6f}",
                            "sat": f"{metrics['delta_saturation_fraction']:.2%}",
                        }
                        diagnostic = metrics.get("diagnostic_total_loss", float("nan"))
                        if np.isfinite(diagnostic):
                            postfix["fixed_diag"] = f"{diagnostic:.6f}"
                        optimization_progress.set_postfix(postfix, refresh=False)
                ...
                optimization_groups.append(
                    {
                        ...
                        "optimization_history": attack_result.history,
                        "selected_step": attack_result.selected_step,
                        "selected_diagnostic_loss": (
                            attack_result.selected_diagnostic_loss
                        ),
