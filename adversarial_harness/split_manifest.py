...
def _canonical_csv_sha256(path: Path) -> str:
    """Hash CSV text after normalizing platform line endings to LF."""
...
def create_matched_split_manifest(
    ...
    canonical_csv_digest = _canonical_csv_sha256(csv_output)
    metadata: Dict[str, object] = {
        "protocol_version": MATCHED_SPLIT_PROTOCOL,
        "dataset": mode,
        "dataset_description": {
        ...
        # csv_sha256 is canonical so it remains stable after Git checkout on
        # Windows or Linux. Keep the generated byte hash for forensic auditing.
        "csv_sha256": canonical_csv_digest,
        "csv_canonical_sha256": canonical_csv_digest,
        "csv_raw_sha256": csv_digest,
...
def load_matched_split_manifest(
    ...
    canonical_csv_digest = _canonical_csv_sha256(csv_input)
    if metadata.get("csv_sha256") != csv_digest:
        if metadata.get("csv_canonical_sha256") != canonical_csv_digest:
            raise ValueError(
                "Split manifest CSV hash does not match its metadata JSON"
            )
    ...
    return LoadedSplitManifest(
        ...
        csv_sha256=(
            canonical_csv_digest
            if metadata.get("csv_canonical_sha256") is not None
            else csv_digest
        ),
    )
